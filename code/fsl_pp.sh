...
if [ $# -lt 2 ]; then
    ...
    ${FSLDIR}/bin/slicetimer -i $bold -o ${outdir}/prefiltered_func_data_st -r 
${TR} --odd;

    echo "Motion correction";
    ${FSLDIR}/bin/mcflirt -in ${outdir}/prefiltered_func_data_st -out 
${outdir}/prefiltered_func_data_mcf -plots -refvol 0 -verbose 0;

    echo "Brain Extraction";
    ${FSLDIR}/bin/fslmaths ${outdir}/prefiltered_func_data_mcf -Tmean 
${outdir}/pmeanfunc;
    ${FSLDIR}/bin/bet2 ${outdir}/pmeanfunc ${outdir}/mask -f 0.35 -n -m;
    ${FSLDIR}/bin/immv ${outdir}/mask_mask ${outdir}/mask;
    ${FSLDIR}/bin/fslmaths ${outdir}/prefiltered_func_data_mcf -mas 
${outdir}/mask ${outdir}/prefiltered_func_data_bet;

    echo "Filtered func data";
    ${FSLDIR}/bin/fslmaths ${outdir}/prefiltered_func_data_bet 
${outdir}/filtered_func_data;
    ${FSLDIR}/bin/fslmaths ${outdir}/filtered_func_data -Tmean 
${outdir}/meanfunc;
    ...
    ${FSLDIR}/bin/fast -t 1 -n 3 -H 0.1 -I 4 -l 20 -B -g -o 
${reg_dir}/${str_bn}_brain ${reg_dir}/${str_bn}_brain
    ${FSLDIR}/bin/immv ${reg_dir}/${str_bn}_brain_restore.nii.gz 
${reg_dir}/${str_bn}_brain.nii.gz
    ...
    ${FSLDIR}/bin/flirt -ref $highres_brain -in $meanfunc -out 
${reg_dir}/meanfunc2highres -omat ${reg_dir}/meanfunc2highres.mat -cost 
corratio -dof 6 -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -interp 
trilinear;

    echo "Registration... inverse: highres to meanfunc";
    ${FSLDIR}/bin/convert_xfm -inverse -omat ${reg_dir}/highres2meanfunc.mat 
${reg_dir}/meanfunc2highres.mat;

    echo "Registration... highres to standard";
    ${FSLDIR}/bin/flirt -ref $mni152_brain -in $highres_brain -out 
${reg_dir}/highres2standard -omat ${reg_dir}/highres2standard.mat -cost 
corratio -dof 12 -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -interp 
trilinear;

    echo "Registration... inverse: standard to highres";
    ${FSLDIR}/bin/convert_xfm -inverse -omat ${reg_dir}/standard2highres.mat 
${reg_dir}/highres2standard.mat;

    echo "Registration... meanfunc to standard";
    ${FSLDIR}/bin/convert_xfm -omat ${reg_dir}/meanfunc2standard.mat -concat 
${reg_dir}/highres2standard.mat ${reg_dir}/meanfunc2highres.mat;

    echo "Registration... inverse: standard to meanfunc";
    ${FSLDIR}/bin/convert_xfm -omat ${reg_dir}/standard2meanfunc.mat -inverse 
${reg_dir}/meanfunc2standard.mat;


    echo "Registration... Non-linear";
    echo "Registration... highres to standard";
    ${FSLDIR}/bin/fnirt --in=${highres_head} 
--aff=${reg_dir}/highres2standard.mat --cout=${reg_dir}/highres2standard_warp 
--iout=${reg_dir}/highres2standard_fnirt --jout=${reg_dir}/highres2standard_jac 
--config=T1_2_MNI152_2mm --ref=${mni152_head} --refmask=${standard_mask} 
--warpres=10,10,10;

    echo "Registration... apply transformations to meanfunc";
    ${FSLDIR}/bin/applywarp --ref=${mni152_brain} --in=${meanfunc} 
--out=${reg_dir}/meanfunc2standard_w --warp=${reg_dir}/highres2standard_warp 
--premat=${reg_dir}/meanfunc2highres.mat;

    echo "Registration... apply transformations to highres";
    ${FSLDIR}/bin/applywarp --ref=${mni152_brain} --in=${highres_brain} 
--out=${reg_dir}/highres2standard_w --warp=${reg_dir}/highres2standard_warp;

    echo "Checking registration and graphing it";

    #Check registration figures
    font=/usr/share/fonts/TTF/DejaVuSans-Bold.ttf;
    ${FSLDIR}/bin/slicer ${reg_dir}/meanfunc2highres.nii.gz ${highres_brain} -a 
${reg_dir}/func_highres.png;
    ${FSLDIR}/bin/slicer -e 0.1 ${reg_dir}/highres2standard_w.nii.gz 
${mni152_brain}  -a ${reg_dir}/highres_standard.png;
    ${FSLDIR}/bin/slicer -e 0.1 ${reg_dir}/meanfunc2standard_w ${mni152_brain} 
-a ${reg_dir}/func_standard.png;

    #Montage of the figures
    ${FSLDIR}/bin/montage -quality 60 -font ${font} -fill white -label '%f' 
${reg_dir}/func_highres.png ${reg_dir}/highres_standard.png 
${reg_dir}/func_standard.png -tile 1x3 -background '#000000' -geometry '480' 
${reg_dir}/checkreg_${bold_bn}.jpg;
    ...
    awk '{print $4}' ${outdir}/prefiltered_func_data_mcf.par | awk '{print 
$0-p}{p=$0}' | awk '{ print ($1 >= 0) ? $1 : 0 - $1}' > 
${outdir}/preFD_mm_04.txt;

    awk '{print $5}' ${outdir}/prefiltered_func_data_mcf.par | awk '{print 
$0-p}{p=$0}' | awk '{ print ($1 >= 0) ? $1 : 0 - $1}' > 
${outdir}/preFD_mm_05.txt;

    awk '{print $6}' ${outdir}/prefiltered_func_data_mcf.par | awk '{print 
$0-p}{p=$0}' | awk '{ print ($1 >= 0) ? $1 : 0 - $1}' > 
${outdir}/preFD_mm_06.txt;

    paste ${outdir}/preFD_mm_04.txt ${outdir}/preFD_mm_05.txt 
${outdir}/preFD_mm_06.txt | awk '{print sqrt($1*$1+$2*$2+$3*$3)}' > 
${QMov_dir}/FD_RMSdisp.txt;

    rm ${outdir}/preFD_mm*;

    awk -v var="$FDthreshold" '{if ($1 > var ) print 1; else print 0}' 
${QMov_dir}/FD_RMSdisp.txt > ${QMov_dir}/FD_peaks.txt;

    cat ${QMov_dir}/FD_peaks.txt | grep 1 | wc -l > 
${QMov_dir}/NVols_FDpeaks.txt;

    echo "Making PNG image of movement";
    awk -v var="$FDthreshold" '{print $1*0+var}' ${QMov_dir}/FD_RMSdisp.txt > 
${QMov_dir}/thresh.txt;
    paste ${QMov_dir}/FD_RMSdisp.txt ${QMov_dir}/thresh.txt > 
${QMov_dir}/FD_RMSdisp_thresh.txt;
    rm -f ${QMov_dir}/thresh.txt;


    ${FSLDIR}/bin/fsl_tsplot -i ${outdir}/prefiltered_func_data_mcf.par -t 
'MCFLIRT estimated rotations (radians)' -u 1 --start=1 --finish=3 -a x,y,z -w 
640 -h 144 -o ${QMov_dir}/rot.png;
    ${FSLDIR}/bin/fsl_tsplot -i ${outdir}/prefiltered_func_data_mcf.par -t 
'MCFLIRT estimated translations (mm)' -u 1 --start=4 --finish=6 -a x,y,z -w 640 
-h 144 -o ${QMov_dir}/trans.png;
    ${FSLDIR}/bin/fsl_tsplot -i ${QMov_dir}/FD_RMSdisp_thresh.txt -t 'Framewise 
Displacement relative RMS DisplacementsOnly (mm)' -u 1 --start=1 --finish=2 -a 
FD,threshold -w 640 -h 144 -o ${QMov_dir}/FDrelRMSd.png;

    ${FSLDIR}/bin/montage ${QMov_dir}/FDrelRMSd.png ${QMov_dir}/rot.png 
${QMov_dir}/trans.png -tile x3 -geometry 640x144+2+2 ${QMov_dir}/montage.png;

    #Mean and standard deviation
    echo -e "\t\tMean\t\tStDev" > ${QMov_dir}/FDrelRMSd_mean_std.txt;
    awk '{ if ($1 > 0) print $1}' ${QMov_dir}/FD_RMSdisp_thresh.txt | awk 
'{delta = $1 - avg; avg += delta / NR; mean2 += delta * ($1 - avg); } END { 
print "FD \t\t"avg"\t\t"sqrt(mean2 / NR); }' >> 
${QMov_dir}/FDrelRMSd_mean_std.txt;
    ...
    ${FSLDIR}/bin/fslmaths ${reg_dir}/${str_bn}_brain_seg_0 -thr 0.99 -bin -ero 
${conf_dir}/sub_CSF;
    ${FSLDIR}/bin/fslmaths ${reg_dir}/${str_bn}_brain_seg_2 -thr 0.99 -bin -ero 
${conf_dir}/sub_WM;
    ${FSLDIR}/bin/fslmaths ${conf_dir}/sub_CSF -add ${conf_dir}/sub_WM 
${conf_dir}/sub_CSFWM;

    echo "Taking segmentation masks to functional space";
    for i in $(ls ${conf_dir}/sub_*); do 
        j=$(basename $i .nii.gz); ${FSLDIR}/bin/flirt -in ${i} -applyxfm -init 
${reg_dir}/highres2meanfunc.mat -out ${conf_dir}/${j}_func -paddingsize 0.0 
-interp nearestneighbour -ref ${outdir}/meanfunc; 
    done;

    echo "Extracting Global signal";
    ${FSLDIR}/bin/fslmeants -i ${outdir}/filtered_func_data -o 
${conf_dir}/Global.txt -m ${outdir}/mask;

    echo "Extracting CSF signal";
    ${FSLDIR}/bin/fslmeants -i ${outdir}/filtered_func_data -o 
${conf_dir}/CSF.txt -m ${conf_dir}/sub_CSF_func;

    echo "Extracting WM signal";
    ${FSLDIR}/bin/fslmeants -i ${outdir}/filtered_func_data -o 
${conf_dir}/WM.txt -m ${conf_dir}/sub_WM_func;

    echo "aCompCor: 5 components";
    ${FSLDIR}/bin/fslmeants -i ${outdir}/filtered_func_data -o 
${conf_dir}/aCompCor.txt -m ${conf_dir}/sub_CSFWM_func --eig --order=5;

    rm -f ${outdir}/design.mat;
    ntpoints=$(wc -l < ${conf_dir}/WM.txt);

    #paste columns
    paste -d " " ${conf_dir}/Global.txt ${conf_dir}/CSF.txt ${conf_dir}/WM.txt 
${outdir}/prefiltered_func_data_mcf.par > ${conf_dir}/design_prev.mat;
    paste -d " " ${conf_dir}/CSF.txt ${conf_dir}/WM.txt 
${outdir}/prefiltered_func_data_mcf.par > ${conf_dir}/design_prev_woGSR.mat;


    echo "Making a file with the derivates and squares of confounding 
variables";
    Rscript --vanilla ./code/dp2.R ${conf_dir}/design_prev.mat 
${conf_dir}/dp2_design_prev.mat;
    Rscript --vanilla ./code/dp2.R ${conf_dir}/design_prev_woGSR.mat 
${conf_dir}/dp2_design_prev_woGSR.mat;
...
--ref=${FSLDIR}/data/standard/MNI152_T1_3mm.nii.gz --in=${input} 
--warp=${outdir}/reg/highres2standard_warp.nii.gz --out=${input}_MNI3mm 
--premat=${outdir}/reg/meanfunc2highres.mat --interp=trilinear;
    done
